<x-app-layout>
    <div class="pos-wrapper min-h-screen py-10" x-data="{ tab: 'venta' }">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="pos-card border rounded-3xl p-6 shadow-xl mb-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="pos-subtitle text-sm mt-1">Administra los flujos de dinero de la empresa en una sola
                            interfaz.</p>
                    </div>

                    <div class="pos-tabs-container border p-1.5 rounded-2xl gap-1 shrink-0 flex">
                        <button
                            @click="tab = 'venta'; setTimeout(() => document.getElementById('barcode-scanner-input')?.focus(), 100)"
            ...
            <div x-show="tab === 'venta'" x-transition>
                ...
                <input type="text" id="barcode-scanner-input" class="absolute opacity-0 w-0 h-0 pointer-events-none"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div class="pos-card border rounded-3xl p-6 shadow-xl">
                                <div
                                    ...
                                    <h3 class="pos-section-title text-sm font-bold uppercase tracking-wider">Agregar
                                        Productos a la Orden</h3>
                                    <span
                                        class="inline-flex items-center gap-1.5 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 px-2.5 py-1 rounded-lg text-[11px] font-medium tracking-wide animate-pulse">
                                ...
                                <div class="flex flex-col lg:flex-row gap-4 items-end sm:items-stretch">
                                    ...
                                    <div class="flex-1 relative">
                                        <label
                                            class="block text-[11px] pos-input-label uppercase font-bold tracking-wider mb-1">Buscar
                                            Plato o Bebida</label>

                                        <div id="custom-select-trigger"
                                            class="w-full pos-select-trigger border rounded-xl px-4 py-3 text-sm cursor-pointer flex items-center justify-between transition">
                                            <span id="select-placeholder" class="pos-placeholder-text">-- Selecciona un
                                                producto --</span>
                                            <span class="pos-placeholder-arrow text-xs">▼</span>
                                        </div>

                                        <div id="custom-select-dropdown"
                                            class="hidden absolute left-0 right-0 mt-2 pos-dropdown-menu border rounded-2xl shadow-2xl z-50 max-h-80 flex flex-col overflow-hidden">
                                            <div class="p-2 pos-dropdown-search-box border-b">
                                                <input type="text" id="search-producto"
                                            ...
                                            <div class="overflow-y-auto flex-1 divide-y" id="options-container">
                                                @foreach ($productos as $prod)
                                                    <div class="option-item flex items-center gap-3 px-4 py-2.5 cursor-pointer pos-option-row transition text-sm"
                                                        ...
                                                        data-stock="{{ $prod->stock }}" data-name="{{ $prod->name }}"
                                                        data-barcode="{{ $prod->barcode ?? '' }}">

                                                        <div
                                                            class="w-10 h-10 rounded-lg pos-option-img-box border flex items-center justify-center overflow-hidden shrink-0">
                                                        ...
                                                        <div class="flex-1 min-w-0">
                                                            <p class="pos-option-name font-medium truncate">
                                                                {{ $prod->name }}</p>
                                                            <p class="text-xs pos-option-meta font-mono">
                                                                ${{ number_format($prod->price, 2) }} <span>|</span>
                                                                Stock: {{ $prod->stock }} @if ($prod->barcode)
                                                                    <span>|</span> 🏷️ {{ $prod->barcode }}
                                                                @endif
                                                            </p>
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="w-full sm:w-32">
                                        <label
                                            class="block text-[11px] pos-input-label uppercase font-bold tracking-wider mb-1">Cantidad</label>
                                    ...
                                    <div class="w-full sm:w-auto flex items-end">
                                        <button type="button" id="btn-agregar"
                                            class="w-full sm:w-auto pos-btn-add text-white px-6 py-3 rounded-xl font-medium text-sm transition h-[46px] shadow-lg">
                                            ＋ Agregar
                                        </button>
                                    </div>
                            ...
                            <div class="pos-card border rounded-3xl shadow-xl overflow-hidden">
                                <div class="px-6 py-4 pos-table-header border-b">
                                    <h3 class="text-sm font-bold uppercase tracking-wider pos-section-title">Detalle del
                                        Consumo</h3>
                                </div>
                                <div class="overflow-x-auto">
                                    <table class="w-full text-left border-collapse pos-table">
                                        <thead>
                                            <tr
                                                class="border-b text-xs uppercase tracking-wider font-semibold pos-table-th-row">
                        ...
                        <div class="space-y-6">
                            <div class="pos-card border rounded-3xl p-6 shadow-xl space-y-6">
                                ...
                                <div>
                                    <label
                                        class="block text-xs font-bold uppercase tracking-wider mb-2 pos-section-title">Asociar
                                        Cliente Registrado</label>
                                    <select name="user_id"
                                        class="w-full pos-input border rounded-xl px-4 py-3 text-sm outline-none">
                                    ...
                                    <p class="text-[11px] pos-helper-text mt-1.5">Si se deja en blanco, la factura no se
                                        asociará a ninguna cuenta de usuario.</p>
                                </div>

                                <div>
                                    <label
                                        class="block text-xs font-bold uppercase tracking-wider mb-2 pos-section-title">Método
                                        de Pago</label>
                                ...
                                <div class="pos-totals-box border p-4 rounded-2xl space-y-3">
                                    ...
                                    <!-- 👈 NUEVA FILA PARA EL IMPUESTO -->
                                    <div class="flex justify-between text-xs pos-helper-text">
                                        <span>IVA ({{ $porcentajeIva }}%)</span>
                                        <span id="txt-impuesto" class="font-mono">$0.00</span>
                                    </div>
                                    ...
                                    <div class="flex justify-between items-center pt-3 border-t pos-totals-divider">
                                        ...
                                        <span id="txt-total"
                                            class="text-xl font-black pos-text-total font-mono">$0.00</span>
                                    </div>
                                </div>

                                <button type="submit"
                                    class="w-full pos-btn-invoice text-white p-4 rounded-xl font-bold text-sm transition flex items-center justify-center gap-2">
                ...
                <div class="pos-card border rounded-3xl p-8 shadow-xl" x-data="{ tipo: 'egreso', concepto: 'pago_proveedor' }">
                    ...
                    <form action="{{ route('admin.movimientos.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label
                                    class="block pos-section-title text-xs font-bold uppercase tracking-wider mb-2">Tipo
                                    de Flujo</label>
                            ...
                            <div>
                                <label
                                    class="block pos-section-title text-xs font-bold uppercase tracking-wider mb-2">Concepto
                                    del Movimiento</label>
                                <select name="concepto" x-model="concepto"
                                    class="w-full pos-input border rounded-xl p-3 text-sm focus:outline-none">
                                    <optgroup label="Egresos Disponibles" x-show="tipo === 'egreso'">
                                        <option value="pago_proveedor">📦 Gasto en Producto (Suma Stock + Proveedor)
                                        </option>
                        ...
                        <div x-show="tipo === 'egreso' && concepto === 'pago_proveedor'" x-transition
                            class="p-5 pos-automation-box border rounded-2xl space-y-4">
                            <p class="pos-text-brand text-xs font-bold uppercase tracking-wide">📦 Automatización de
                                Inventario</p>

                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <label class="block pos-helper-text text-xs mb-1">Proveedor Asociado</label>
                                    <select id="movProveedor" name="proveedor_id"
                                        class="w-full pos-input-nested border rounded-xl p-2.5 text-xs focus:outline-none">
                                        <option value="">Seleccione Proveedor...</option>
                                        @foreach ($proveedores as $prov)
                                            <option value="{{ $prov->id }}">
                                                {{ $prov->company_name ?? $prov->nombre }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="block pos-helper-text text-xs mb-1">Producto a Surtir</label>
                                    <select id="movProducto" name="producto_id"
                                        class="w-full pos-input-nested border rounded-xl p-2.5 text-xs focus:outline-none">
                                        <option value="" data-proveedor="">Seleccione Producto...</option>
                                        @foreach ($productos as $prod)
                                            <option value="{{ $prod->id }}"
                                                data-proveedor="{{ $prod->supplier_id ?? '' }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="md:col-span-1">
                                <label
                                    class="block pos-section-title text-xs font-bold uppercase tracking-wider mb-2">Valor
                                    / Monto ($)</label>
                            ...
                            <div class="md:col-span-2">
                                <label
                                    class="block pos-section-title text-xs font-bold uppercase tracking-wider mb-2">Descripción
                                    / Concepto Detallado</label>
                                <input type="text" name="descripcion" required
                                    placeholder="Ej: Compra de insumos o pago de servicios"
                                    class="w-full pos-input border rounded-xl p-3 text-sm focus:outline-none">
                            </div>
                        </div>

                        <button type="submit"
                            class="w-full pos-btn-invoice font-black py-4 rounded-xl transition duration-200 text-sm uppercase tracking-wide shadow-lg flex items-center justify-center gap-2">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const porcentajeIva = {{ $porcentajeIva }};
            ...
            const txtImpuesto = document.getElementById('txt-impuesto'); // 👈 Agregar esta constante
            ...
            function focusScanner() {
                ...
                const inputsIgnorados = ['search-producto', 'cantidad-producto', 'user_id', 'metodo_pago',
                    'proveedor_id', 'producto_id', 'cantidad_producto', 'monto', 'descripcion'
                ];

                if (activeEl && inputsIgnorados.includes(activeEl.id) || activeEl.name && inputsIgnorados.includes(
                        activeEl.name)) {
                    return;
                }
            ...
            barcodeScannerInput.addEventListener('keydown', function(e) {
                if (e.key === 'Enter') {
                    ...
                    const matchedOption = optionsContainer.querySelector(
                        `.option-item[data-barcode="${scannedCode}"]`);
            ...
            function inyectarProductoDirecto(prodId, prodNombre, prodPrecio, prodStock, cantidad) {
                ...
                if (filaExistente) {
                    ...
                    if (nuevaCant > prodStock) {
                        alert(
                            `Al acumular las unidades de "${prodNombre}" superas el stock disponible (${prodStock}).`
                        );
                        return;
                    }
                    ...
                    filaExistente.querySelector('.txt-subtotal-fila').textContent =
                        `$${(prodPrecio * nuevaCant).toFixed(2)}`;
                    calcularGranTotal();
                    return;
                }
            ...
            optionsContainer.addEventListener('click', function(e) {
                ...
                selectDropdown.classList.add('hidden');
                searchInput.value = '';
                optionsContainer.querySelectorAll('.option-item').forEach(item => item.style.setProperty(
            ...
            function calcularGranTotal() {
                ...
                document.querySelectorAll('.struct-item-row').forEach(function(fila) {
                    subtotal += parseFloat(fila.querySelector('.txt-subtotal-fila').textContent.replace('$',
                        ''));
                });

                const montoImpuesto = subtotal * (porcentajeIva / 100);
                const totalFinal = subtotal + montoImpuesto;

                txtSubtotal.textContent = `$${subtotal.toFixed(2)}`;
                txtImpuesto.textContent = `$${montoImpuesto.toFixed(2)}`; // 👈 Muestra el desglose del IVA
                txtTotal.textContent = `$${totalFinal.toFixed(2)}`;
            }
            ...
            if (movProveedor && movProducto) {
                movProveedor.addEventListener('change', function() {
                    ...
                    Array.from(movProducto.options).forEach(option => {
                        ...
                        const prodDataProv = option.getAttribute('data-proveedor') ? option
                            .getAttribute('data-proveedor').toString().trim() : "";
                ...
                movProducto.addEventListener('change', function() {
                    ...
                    const prodDataProv = opcionSeleccionada.getAttribute('data-proveedor') ?
                        opcionSeleccionada.getAttribute('data-proveedor').toString().trim() : "";
                    if (prodDataProv) {
