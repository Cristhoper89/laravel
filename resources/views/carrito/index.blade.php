        ...
        <div class="max-w-5xl mx-auto px-6">
            ...
            <div class="feature-card border rounded-3xl p-8 shadow-xl mb-8 flex justify-between items-center">
                ...
                <a href="/cliente/dashboard"
                    class="db-link-action font-semibold text-sm transition duration-200 no-underline flex items-center gap-1">
                    ← Volver al Menú
                </a>
            </div>
            ...
            @if (session('success'))
                <div
                    class="mb-6 bg-emerald-950/20 border border-emerald-500/30 text-emerald-400 rounded-2xl p-4 shadow-md flex items-center gap-2">
                    <span>✨</span>
            ...
            @if (session('error'))
                <div
                    class="mb-6 bg-rose-950/20 border border-rose-500/30 text-rose-400 rounded-2xl p-4 shadow-md flex items-center gap-2">
                    <span>⚠️</span>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        @foreach ($carrito as $id => $item)
                            ...
                            <div
                                class="feature-card border rounded-2xl p-4 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4 transition duration-200">
                                <div class="flex items-center gap-4 w-full sm:w-auto">
                                    <div
                                        class="h-16 w-16 bg-black/20 rounded-xl overflow-hidden flex-shrink-0 border border-white/5">
                                        @if ($item['image'])
                                            <img src="{{ Str::startsWith($item['image'], 'http') ? $item['image'] : asset('storage/' . $item['image']) }}"
                                                alt="{{ $item['name'] }}" class="w-full h-full object-cover">
                                        @else
                                            <div
                                                class="w-full h-full flex items-center justify-center text-lg bg-black/10">
                                                🍽️</div>
                                        @endif
                                    </div>
                                    <div>
                                        <h4 class="text-white font-bold text-lg">{{ $item['name'] }}</h4>
                                        <p class="text-slate-500 text-xs">Precio unitario: <span class="font-bold"
                                                style="color: var(--color-success);">${{ number_format($item['price'], 2) }}</span>
                                        </p>
                                    </div>
                                </div>

                                <div
                                    class="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto border-t sm:border-t-0 pt-3 sm:pt-0 border-white/5">
                                    ...
                                    <div class="text-right min-w-[80px]">
                                        <p class="text-white font-black text-sm">${{ number_format($totalPlato, 2) }}
                                        </p>
                                    </div>

                                    {{-- Formulario para eliminar del carrito --}}
                                    <form action="{{ route('carrito.remove') }}" method="POST"
                                        onsubmit="return confirm('¿Remover este platillo de la orden?')">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="id" value="{{ $id }}">
                                        <button type="submit"
                                            class="text-rose-400 hover:text-rose-500 transition duration-200 p-2 text-sm">
                                            ❌
                                        </button>
                                    </form>
                                </div>
                    ...
                    <div class="feature-card border rounded-3xl p-6 shadow-xl h-fit sticky top-6">
                        <h3 class="text-xl font-bold text-white mb-4 pb-2 border-b border-white/5">Resumen de Cuenta
                        </h3>
                        ...
                        <div class="space-y-3 text-sm">
                            ...
                            <div class="flex justify-between text-slate-400">
                                <span>Impuesto ({{ $porcentajeIva }}%)</span>
                                <span class="text-white font-semibold">${{ number_format($montoIva, 2) }}</span>
                            </div>
                            ...
                            <div class="flex justify-between text-base pt-3 border-t border-white/5">
                                ...
                                <span class="text-2xl font-black"
                                    style="color: var(--color-success);">${{ number_format($total, 2) }}</span>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('facturas.store') }}" method="POST" class="mt-6 space-y-4">
                            ...
                            @foreach ($carrito as $id => $item)
                                <input type="hidden" name="productos[{{ $id }}][id]"
                                    value="{{ $id }}">
                            ...
                            <div>
                                <label
                                    class="block text-slate-400 mb-2 text-xs font-medium uppercase tracking-wider">Método
                                    de Pago</label>
                                <select name="metodo_pago"
                                    ...
                                    <option value="Transferencia" class="bg-slate-900">Transferencia Bancaria 🏦
                                    </option>
                                </select>
                            </div>
            ...
            @else
                <div class="feature-card border rounded-3xl p-16 text-center shadow-xl">
                    ...
                    <p class="text-slate-400 text-sm max-w-sm mx-auto mb-6">Parece que aún no has seleccionado ningún
                        platillo de nuestro menú del día.</p>
                    <a href="/cliente/dashboard"
